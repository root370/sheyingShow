  ...
  return (
    <div className="fixed z-50 top-7 left-6 md:top-8 md:left-1/2 md:-translate-x-1/2">
      ...
      <div className="md:hidden bg-[#151515] backdrop-blur-xl border border-white/10 rounded-full p-1 pl-4 pr-1 flex items-center shadow-2xl gap-4">
        <Link href="/dashboard" className="group relative outline-none">
          <div className={`relative z-10 flex flex-col items-start leading-none py-2 px-1 transition-colors duration-300 ${currentMode === 'dashboard' ? 'text-black' : 'text-white/70 group-hover:text-white'}`}>
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase">我的</span>
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase">展览</span>
          </div>
        ...
        <Link href="/" className="relative outline-none">
           <div className={`relative z-10 px-6 py-3 rounded-full text-xs font-sans font-bold tracking-[0.2em] uppercase transition-colors duration-300 ${currentMode === 'explore' ? 'text-black' : 'text-white/70 hover:text-white'}`}>
             SCOUT
           </div>
           {currentMode === 'explore' && (
            <motion.div
      ...
      <div className="hidden md:flex bg-white/5 backdrop-blur-xl border border-white/10 rounded-full p-1 shadow-2xl">
        <Link href="/dashboard" className="relative px-8 py-3 rounded-full text-xs font-sans font-bold tracking-[0.2em] transition-colors duration-500 outline-none focus-visible:ring-1 focus-visible:ring-white/30 uppercase">
          {currentMode === 'dashboard' && (
            <motion.div
        ...
        <Link href="/" className="relative px-8 py-3 rounded-full text-xs font-sans font-bold tracking-[0.2em] transition-colors duration-500 outline-none focus-visible:ring-1 focus-visible:ring-white/30 uppercase">
          {currentMode === 'explore' && (
            <motion.div
              layoutId="nav-pill-desktop"
              className="absolute inset-0 bg-white rounded-full shadow-lg"
